...
@section('title') Final Approval Projection @endsection
                    ...
                    <div class="d-inline"> 
                       <h4 class="mt-0 fw-bolder ms-0">
                          <a class="fw-bold fs--1 ms-3 mx-2" href="{{route('approvals')}}"> 
                            ...
                            <span class="text-500"> Projection For Final Approval  - </span> {{ $fname}}
...
<div class="for_approval_projection_final_edit_card row mb-2">
            ...
            <div class="card border-0 p-3" >  
                <div class="" style="height:60vh; max-height:350vh;min-width:70vh;overflow-y:auto;overflow-x:hidden;">              
                    <div class="ms-n1 ps-1 scrollbar">
                        ...
                        <table id="for-approval-projection-final-customer-list-table" class="fs--1 table  table-responsive text-center">
                            <thead class="border border-1 sticky-top">
                                <tr role="row">
                                    <th colspan="2" class="bg-white  text-center" rowspan="1">&nbsp</th>
                                    
                                    <th colspan="3" class="bg-white  border-end top text-center" rowspan="1">Projection</th>
                                ...
                                <tr>
                                    ...
                                    <th scope="col" class="text-center" width="3%">
                                        <div class="d-flex ">
                                            <input class="form-check-input for_approval_checkfinalallcustomerisbn" type="checkbox" value="">
                                        </div>
                                    </th>
                                </tr>
                            </thead>
  ...
  <div class="input-group mt-0 d-none un-cl  w-75 pt-3 mt-3">
    <form class="submit_approve_projection_final d-none un-cl b-form " readonly="readonly" method="POST">
        @csrf
...
<div class="offcanvas offcanvas-end content-offcanvas border offcanvas-backdrop-transparent border-start border-300 shadow-none bg-100" style="width:60rem !important;" id="MyForApprovalProjectionISBNListOffCanvas" tabindex="-1" aria-labelledby="offcanvasLeftLabel">
    <div class="offcanvas-header pb-0">
       <h5 id="offcanvasLeftLabel">
            <span class="fw-bold text-900 approvalsfinal_isbnlist_customername"> - 
                
                
            </span><span class="isbnlist_customer_bsastatus"></span>
                </br>
                    <span class="fw-bold text-600 approvalsfinal_isbnlist_customercode">- </span>    
                    <input class="d-none un-cl form-control approvals_isbnlist_docnum_val" readonly="readonly">
                    <input class="d-none un-cl form-control approvalsfinal_isbnlist_customercode_val" readonly="readonly">
                    <input class="d-none un-cl form-control approvalsfinal_isbnlist_customername_val" readonly="readonly">
...
function ISBNTablefindIsbnEditRow(docnum, isbn) {
    return $(".isbntable tbody tr").filter(function () {
        var $tr = $(this);
        var dcnum = ($tr.find(".for_approval_projection_final_edit_isbn_docnum").val() || "").trim();
        var code = ($tr.find(".for_approval_projection_final_edit_isbn_customercode").val() || "").trim();
        var vIsbn = ($tr.find(".for_approval_projection_final_edit_isbn").val() || "").trim();
...
function customerISBNListDisplay (basedocnum,username,docnum){
    ...
    $.ajax({
                url:"/datatable_for_approval_projection_final_customer_isbn_list?basedocnum="+basedocnum+"&name="+username+"&docnum="+docnum, 
                type:'GET',
                headers: {
                ...
                beforeSend: function() {
                    ...
                    // showLoadingDiv('.for_approval_projection_final_edit_card'),setTimeout( () => hideLoadingDiv('.for_approval_projection_final_edit_card'),1000);
                ...
                success:function(data){
                    ...
                    if (cisbnli && cisbnli.length > 0)  {
                        ...
                        for_approval_projection_final_edit_customer_list(basedocnum,username)
                    }
                ...
                error:function(data){
                        ...
                        hideLoadingDiv('.for_approval_projection_final_edit_card');
...
var defaultProjectionQtyClass = $('.for_approval_projection_final_projtn_qty');
...
// if(className === 1) {
    ...
    defaultProjectionQtyClass.each( function() {
        ...
        var docnum = trClosest.find('.for_approval_projection_final_isbn_docnum').val();
        var ssmqty = trClosest.find('.for_approval_projection_final_ssm_qty').val();
        var isbn = trClosest.find('.for_approval_projection_final_isbn').val();
        var title = trClosest.find('.for_approval_projection_final_isbn_title').val();
        var disc = trClosest.find('.for_approval_projection_final_isbn_disc').val();
        var populationqty = trClosest.find('.for_approval_projection_final_population_qty').val();
        var unitp = trClosest.find('.for_approval_projection_final_isbn_unitp').val();
        var lastyearsale = trClosest.find('.for_approval_projection_final_isbn_prev1_sales').val();
        ...
        var projtntotalvalue =  $(`.for_approval_projection_final_projtn_qty[data-customercode="${customercode}"]`);
        ...
        var approve = $('.'+ docnum + isbn + 'finalapprove').val();
        $('.'+ docnum + isbn + 'editapproveqty').val(ssmqty);

                    
        if(approve === '1') {
            // $('.'+ docnum + isbn + 'finaleditisbnrow' ).addClass('bg-success-100')
            $('.'+ docnum + isbn + 'checkfinal' ).prop('checked',true)
        }
        else {
            // $('.'+ docnum + isbn + 'finaleditisbnrow' ).removeClass('bg-success-100')
            $('.'+ docnum + isbn + 'checkfinal' ).prop('checked',false)
        }
...
function for_approval_projection_final_edit_customer_list(projdocnum,username) {

$.ajax({
       url:"/datatable_for_approval_projection_final_customer_list?basedocnum="+projdocnum+"&username="+username, 
       type:'GET',
       headers: {
               'X-CSRF-TOKEN': getCsrfToken() 
       },
       beforeSend: function() {
        ...
        $('#for-approval-projection-final-customer-list-table tbody tr').empty()
        $('.submit_approve_projection_final tr').remove()
        // showLoadingDiv('.for_approval_projection_final_edit_card'),setTimeout( () => hideLoadingDiv('.for_approval_projection_final_edit_card'),1000);

            showLoadingDiv('.for_approval_projection_final_edit_card');
           ...
           console.log(data);
            ...
            if(customerlist[0].systemstatus == '2-2') {
                ...
                for(i=0;i<customerlist.length;i++) {
                        ...
                        var checkfinalcustomerisbn = d.checkfinalcustomerisbn;
                ...
                $('#for-approval-projection-final-customer-list-table tbody').append(aa)
            ...
            if (prjd && prjd.length > 0) {
                ...
                for(p=0;p<prjd.length;p++){
                    ...
                    var linetotal =  prjd[p].linetotal   
                        ...
                        iprjl += `
                    <tr class="input-group form-control">
                        <td>
                        
                                <input name="forapproval_projection_final_docnum[]" value="`+docnum+`" class="for_approval_projection_final_isbn_docnum">
                                <input name="forapproval_projection_final_isbn_approve[]" class="for_approval_projection_final_isbn_approve `+docnum+isbn+`finalapprove `+docnum+`finalapprovealldocnum">
                                <input name="forapproval_projection_final_customercode[]" value="`+customercode+`" class="for_approval_projection_final_isbn_customercode">
                                <input name="forapproval_projection_final_branchwhouse[]" value="`+branchwhouse+`" class="for_approval_projection_final_branchwhouse">
                                <input name="forapproval_projection_final_isbn[]" value="`+isbn+`" class="for_approval_projection_final_isbn">
                                <input name="forapproval_projection_final_isbn_title[]" value="`+title+`" class="for_approval_projection_final_isbn_title">
                                <input name="forapproval_projection_final_isbn_unitp[]" value="`+unitpInt+`" class="for_approval_projection_final_isbn_unitp">
                                <input name="forapproval_projection_final_isbn_disc[]" value="`+disc+`" class="for_approval_projection_final_isbn_disc">
                                <input name="forapproval_projection_final_isbn_population[]" value="`+populationqtyInt+`" class="for_approval_projection_final_population_qty">
                                <input name="forapproval_projection_final_isbn_qty[]" value="`+projectionQty+`" class="for_approval_projection_final_projtn_qty" data-customercode="`+customercode+`">
                                <input name="forapproval_projection_final_isbn_rsm_qty[]" value="`+projectionQty+`" class="for_approval_projection_final_rsm_qty `+customercode+isbn+`rsmqty `+docnum+isbn+`rsmqty" data-customercode="`+customercode+`">
                                <input name="forapproval_projection_final_isbn_ssm_qty[]" value="`+projectionQty+`" class="for_approval_projection_final_ssm_qty `+customercode+isbn+`ssmqty" data-customercode="`+customercode+`">
                                <input name="forapproval_projection_final_isbn_linetotal[]" value="`+linetotal+`" class="for_approval_projection_final_isbn_linetotal `+customercode+`linetotal `+customercode+isbn+`linetotal `+docnum+isbn+`linetotal">
                ...
                $('.submit_approve_projection_final').append(iprjl);
...
$(document).on('click','.for_approval_projection_final_edit_isbn_approve_check',function (e) {

    var docnum = $(this).data('docnum');
    var customercode = $(this).data('customercode');
    var isbn = $(this).data('isbn');

    var $existing = $('.' + docnum + isbn + 'finaleditisbnrow');
        ...
        $('.'+docnum+isbn+'finalapprove').val(v)
...
$(document).on('submit', '.submit_return_projection', function(e) {
        ...
        var docnumlist = $('.for_approval_projection_final_edit_customerisbn_approve_check:checked').map( function () { 
...
$(document).on('click','.for_approval_checkfinalallcustomerisbn',function (e) {

   
    var editcustomerisbncheck= $('.for_approval_projection_final_edit_customerisbn_approve_check') 
    ...
    $('.for_approval_projection_final_isbn_approve').val(v)

});

$(document).on('click','.for_approval_projection_final_edit_customerisbn_approve_check',function (e) {
    ...
    if($(this).is(':checked')){
        ...
        $('.'+docnum+'finalapprovealldocnum').val(v)
...
$(document).on('click','.forfinalapproval_approve_isbn',function (e) {
...
var titled = $(this).data('title')
var customercode = $(this).data('customercode')
var approveQty = $(this).closest("tr").find('.for_approval_projection_final_edit_approve_qty').val()
var unitp = $(this).closest("tr").find('.for_approval_projection_final_edit_isbn_unitp').val()
...
.then((willCancel) => {
    ...
    if (willCancel) {
            ...
            $.ajax({
                    url:"/approve_projection_final_isbn", 
                    data: {
                        id : id,
                        docnum : docnum,
...
$(document).on('click','.btn-approve',function (e) {
   ...
   if ($('.for_approval_projection_final_isbn_approve').filter( function () {
        return $(this).val() === '1';
    }).length > 0) {
...
$(document).on('click','.for_approval_projection_final_isbn_list_display',function (e) {
            var customercode = $(this).data('customercode');
            ...
            $('.approvalsfinal_isbnlist_customercode').text(customercode)
            $('.approvalsfinal_isbnlist_customername').text(customername)

            $('.approvals_isbnlist_docnum_val').val(docnum)
            $('.approvalsfinal_isbnlist_customercode_val').val(customercode)
            $('.approvalsfinal_isbnlist_customername_val').val(customername)
...
$(document).on('change','.for_approval_projection_final_edit_approve_qty', function(e) {
        ...
        var projectionQty = parseInt(projection) || 0;
        var approveQty = parseInt(v) || 0;

        var unitp = trClosest.find('.for_approval_projection_final_edit_isbn_unitp').val();

        var unitpInt = parseInt(unitp) || 0;
        var linetotal = approveQty * unitpInt
        var linetotalDisplay = numberFormat(linetotal)
        // alert(lastyearsaleIntAllowed) 

        // Scenario 1: Projection qty is more than population qty
        if ( approveQty > projectionQty) {
            $(this).val();
            alert('Error: Qty is more than Projection.');
            $(this).focus();
            return false;
        }
